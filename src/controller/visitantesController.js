...
function validarNome(nome) {
  return typeof nome === "string" && nome.trim().length >= 2 && /^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$/.test(nome.trim());
}
...
endpoints.get('/visitantes/nome/:nome', async (req, resp) => {
  try {
    ...
    if (!visitante || visitante.length === 0) {
      return resp.status(404).send({ erro: "Nenhum visitante encontrado com esse nome." });
    }
    resp.send(visitante);
  } catch (err) {
    console.error(err);
...
endpoints.get('/visitantes/cpf/:cpf', async (req, resp) => {
  try {
    ...
    if (!validarCPF(cpf)) {
      return resp.status(400).send({ erro: "CPF inválido. Digite exatamente 11 números." });
    }
    const visitante = await repo.filtrarPorCpf(cpf);
    if (!visitante || visitante.length === 0) {
      return resp.status(404).send({ erro: "Nenhum visitante encontrado com esse CPF." });
    }
    resp.send(visitante);
  } catch (err) {
    console.error(err);
...
endpoints.post('/visitantes', async (req, resp) => {
  try {
    ...
    if (!validarNome(novo.nm_cadastrado)) {
      return resp.status(400).send({ erro: "Nome inválido. Deve ter pelo menos 2 letras e conter apenas letras e espaços." });
    }

    if (!novo.idade_cadastrado || isNaN(novo.idade_cadastrado) || novo.idade_cadastrado <= 0) {
      return resp.status(400).send({ erro: "Idade inválida. Digite um número maior que 0." });
    }

    if (!novo.cpf_cadastrado || !validarCPF(String(novo.cpf_cadastrado))) {
      return resp.status(400).send({ erro: "CPF inválido. Deve conter exatamente 11 números." });
    }
    ...
    if (novo.telefone_numero && !validarTelefone(String(novo.telefone_numero))) {
      return resp.status(400).send({ erro: "Telefone inválido. Deve conter apenas números (8 a 15 dígitos)." });
    }
...
endpoints.put('/visitantes/:id', async (req, resp) => {
  try {
    ...
    if (novosDados.telefone_numero && !validarTelefone(String(novosDados.telefone_numero))) {
